...
An unavailable or invalid mvhs.io schedule produces **Schedule unavailable** and a retry option. Public source links are provided. Use the school's announced schedule if public feeds disagree with it; a valid feed is not a guarantee that a last-minute change has been recorded. Once the script is published, request details can be read in the comment at the top of `aeries-playground.user.js`.
...
Once the script is published, read the comment at the top of `aeries-playground.user.js` for the saved-data breakdown and removal instructions.
